...
agg_demand <- aggregate_demand(gas_data$price_dollars)
...
predict_df <- data.frame(gas_data$price_dollars, gas_data$q_high_gallons, gas_data$q_low_gallons, quantity_h, graph_h, quantity_l, graph_l, agg_demand) %>% 
...
ggplot(data = predict_df, aes(x = quantity_h, y = graph_h))+
  ...
  geom_line(aes(x = price_q, y = agg_demand), color = "blueviolet") +
  theme_minimal() +
  labs(x = "Price [$/gal]", y = "Quantity ") +
  coord_flip()
